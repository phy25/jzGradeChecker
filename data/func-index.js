...
jzgc.index = {
	...
	renderPage: function(indexData, $dest){
		...
		$('<div id="group-stuinfo"><div class="control-group"><label class="control-label" for="xuehao">学号</label><div class="controls"><input type="tel" pattern="[0-9]*" id="xuehao" name="xuehao" placeholder="五位数班学号" required="required" min="10101" max="32099" tabindex="10" /></div></div><div class="control-group"><label class="control-label" for="password">密码</label><div class="controls"><div class="input-append"><input type="text" id="password" name="password" placeholder="身份证号码等" required="required" tabindex="20" /><button class="btn" type="button" title="密文显示密码" id="password-hide-btn" data-toggle="button" tabindex="25">隐藏</button></div></div></div></div>').appendTo($f);
